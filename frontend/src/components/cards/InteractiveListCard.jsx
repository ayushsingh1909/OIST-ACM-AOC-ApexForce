...
const InteractiveListCard = ({
  ...
  return (
    <div className="bg-white border border-[#111111]/7 rounded-2xl p-6 md:p-8 shadow-[0_4px_30px_rgba(17,17,17,0.015)] flex flex-col gap-6">
      ...
      {/* Header Block */}
      <div className="flex justify-between items-center border-b border-[#111111]/5 pb-4">
        <div className="space-y-1">
          <h3 className="text-sm font-semibold tracking-tight text-[#111111] uppercase font-mono">
            {title}
          </h3>
          {subtitle && (
      ...
      <div ref={listRef} className="overflow-x-auto w-full">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-[#111111]/5">
              {headers.map((h, i) => (
                <th key={i} className="pb-3 text-[10px] font-bold text-[#555555] uppercase tracking-wider font-mono">
                  {h}
                </th>
              ))}
            ...
            {rows.map((row) => {
              ...
              return (
                <React.Fragment key={row.id}>
                  {/* Base Row */}
                  <tr
                    className="stagger-row hover:bg-[#FAFAFA] transition-all duration-200 cursor-pointer"
                    onClick={() => row.details && toggleRow(row.id)}
                  >
                    {row.cells.map((cell, idx) => (
                      <td key={idx} className="py-4 text-xs font-sans text-[#111111]">
                        {cell}
                      </td>
                    ))}
                    {row.details && (
                  ...
                  {row.details && isExpanded && (
                    <tr>
                      <td colSpan={headers.length + (row.details ? 1 : 0)} className="py-4 px-4 bg-[#FAFAFA]/50 border-y border-[#111111]/5">
                        <div className="text-[11px] text-[#555555] leading-relaxed transition-all">
                          {row.details}
                        </div>
                      </td>
                    </tr>
