    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="p-5 bg-white/60 border border-slate-200 rounded-2xl">
          ...
          <p className="text-2xl font-bold text-slate-900">{overview.overallMasteryScore}%</p>
          <p className="text-xs text-slate-500">Overall Mastery</p>
        </div>
        <div className="p-5 bg-white/60 border border-slate-200 rounded-2xl">
          <FiFileText className="w-6 h-6 text-indigo-400 mb-2" />
          <p className="text-2xl font-bold text-slate-900">{overview.resumeScore}</p>
          <p className="text-xs text-slate-500">Resume Score</p>
        </div>
        <div className="p-5 bg-white/60 border border-slate-200 rounded-2xl">
          <FiTarget className="w-6 h-6 text-amber-400 mb-2" />
          <p className="text-2xl font-bold text-slate-900">{overview.quizAvgAccuracy}%</p>
        ...
        <div className="p-5 bg-white/60 border border-slate-200 rounded-2xl">
          ...
          <p className="text-2xl font-bold text-slate-900">{overview.totalQuizAttempts}</p>
          <p className="text-xs text-slate-500">Quizzes Taken</p>
        </div>
      </div>

      <div className="grid lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-2 p-6 bg-white/60 border border-slate-200 rounded-2xl">
          ...
          {highRiskItems.length === 0 ? (
            ...
            <div className="space-y-3">
              {highRiskItems.map((item, i) => (
                <div key={i} className="p-3 bg-slate-50/50 border border-slate-200 rounded-xl">
          ...
          <div className="space-y-2">
            {recommendedActions.map((action, i) => (
              <Link
                key={i}
                to={action.route}
                className="flex items-center justify-between p-3 bg-slate-50/50 border border-slate-200 rounded-xl hover:border-violet-500/30 transition-all"
              >
        ...
        <div className="p-6 bg-white/60 border border-slate-200 rounded-2xl">
          ...
          <div className="space-y-2">
            {upcomingTasks.assignments?.slice(0, 3).map((a) => (
              <div key={a.id} className="p-3 bg-slate-50/50 border border-slate-200 rounded-xl">
            ...
            {upcomingTasks.revisions?.slice(0, 2).map((r, i) => (
              <div key={i} className="p-3 bg-slate-50/50 border border-slate-200 rounded-xl">
                <p className="text-sm font-medium text-slate-900">Review: {r.topic}</p>
      ...
      <div className="p-6 bg-white/60 border border-slate-200 rounded-2xl mb-8">
        <h2 className="text-lg font-semibold text-slate-900 mb-4 flex items-center gap-2">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-3">
          {studyRoadmap.map((day) => (
            <div key={day.date} className="p-3 bg-slate-50/50 border border-slate-200 rounded-xl">
      ...
      <div className="p-4 bg-gradient-to-r from-violet-600/10 to-indigo-600/10 border border-violet-500/20 rounded-xl text-sm text-slate-600">
        <span className="text-violet-400 font-semibold">Adaptive Engine: </span>
        Suggested difficulty: <span className="text-slate-900">{adaptiveFeedback.suggestedDifficulty}</span>
        {adaptiveFeedback.focusTopics?.length > 0 && (
          <> · Focus: <span className="text-slate-900">{adaptiveFeedback.focusTopics.join(", ")}</span></>
        )}
